...
def load_seed_urls():
    collecting_webpage_urls = []
    handle = open('./yahoo_seed_urls.txt', 'r')
    for line in handle:
        line = line.strip('\n')
        collecting_webpage_urls.append(line)
    handle.close()
    return collecting_webpage_urls

def StartCollectYahooAd(browser):
    ...
    collecting_webpage_urls = load_seed_urls()
    to_be_collectd_webpage_urls = []
    current_collected_webpage_urls = []

    # if it is the first time run this experiment
    if len(collecting_webpage_urls) == 0:
        collecting_webpage_urls = ['https://sg.yahoo.com/']         # set seed url
    if len(current_collected_webpage_urls) == 0:
        current_collected_webpage_urls = load_webpage_urls('./exclude_webpage_url_from_other_country.txt')
        ...
        for webpage_url in collecting_webpage_urls:
            ...
            # if True:
                if webpage_url in current_collected_webpage_urls:
                    ...
                    number_for_rest += 1
                    if number_for_rest % 10 == 0:                  # once finished 10 webpages, we restart the browser
                        try:
                            if browser:
                                browser.quit()
                        except:
                    ...
                    # find new urls in current yahoo webpage
                    to_be_collected_webpages_backup = copy.deepcopy(to_be_collectd_webpage_urls)
                    try:
                        to_be_collectd_webpage_urls = findNewURLsInCurrentWebpage(browser, to_be_collectd_webpage_urls, current_collected_webpage_urls)
                    except:
                        to_be_collectd_webpage_urls = copy.deepcopy(to_be_collected_webpages_backup)
...
if __name__ == '__main__':
    os.chdir('/home/chxiang/PycharmProjects/KDD_BOWEI/crawler_1/yahoo')
    browser = RestartBrowser()
    StartCollectYahooAd(browser)
    browser.quit()
